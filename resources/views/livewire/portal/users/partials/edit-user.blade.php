                ...
                <div class="p-2 p-lg-4">
                    ...
                    <x-form-items.form wire:submit="update" class="form-modal">
                        ...
                        <div class="form-group row mb-3">
                            ...
                            <div class='col-md-6 col-xs-12'>
                                ...
                                <select wire:model="status" name="status" class="form-select  @error('status') is-invalid @enderror">
                                    ...
                                    <option value="true" {{$status === true ? 'selected' : ''}}>{{__('Actif')}}</option>
                                    <option value="false" {{$status === false ? 'selected' : ''}}>{{__('Bannir')}}</option>
                                </select>
                                @error('status')
                                <div class="invalid-feedback">{{$message}}</div>
                                @enderror
                        ...
                        <div class="d-flex justify-content-end">
                            <button type="button" wire:click="clearFields" class="btn btn-gray-200 text-gray-600 ms-auto mx-3" data-bs-dismiss="modal">{{__('Fermé')}}</button>
                            <button type="submit" wire:click.prevent="update" class="btn btn-success " wire:loading.attr="disabled">{{__('Modifier')}}</button>
                        </div>
                    </x-form-items.form>
                </div>
            </div>
