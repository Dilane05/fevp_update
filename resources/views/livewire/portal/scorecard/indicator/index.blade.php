<div>
    @include('livewire.portal.scorecard.indicator.create')
    {{-- @include('livewire.admin.contract.edit') --}}
    {{-- @include('livewire.portal.clients.partials.edit-client')
    @include('livewire.portal.clients.partials.import-clients')
            ...
            <div class="mb-lg-0">
                <nav aria-label="breadcrumb" class="d-none d-md-inline-block">
                    <ol class="breadcrumb breadcrumb-dark breadcrumb-transparent">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">{{ __('Indicateurs') }}</li>
                    </ol>
                </nav>
                <h1 class="h3 d-flex justify-content-start align-items-end">
                    ...
                    {{ __('Gestion des indicateurs') }}
                </h1>
            </div>

            <div class="d-flex justify-content-between mb-2">
                {{-- @can('') --}}
                <a href="#" data-bs-toggle="modal" data-bs-target="#CreateIndicatorModal"
            ...
            <table class="table client-table table-hover table-bordered align-items-center">
                <thead>
                    <tr>
                        ...
                        <th class="border-bottom">{{ __('Valeur Min') }}</th>
                        <th class="border-bottom">{{ __('Valeur Max') }}</th>
                        <th class="border-bottom">{{ __('Score Maximal') }}</th>
                        <th class="border-bottom">{{ __('Type de Condition') }}</th>
                ...
                <tbody>
                    @forelse($indicators as $indicator)
                        ...
                        <tr>
                            ...
                            <td>{{ $indicator->min_value }}</td>
                            <td>{{ $indicator->max_value }}</td>
                            <td>{{ $indicator->min_score }}</td>
                            <td>{{ $indicator->max_score }}</td>
                            <td class="text-end">
                                {{ $indicator->created_at? Carbon::parse($indicator->created_at)->locale('fr')->isoFormat('LL'): '' }}
                            </td>
                            {{-- @canany(['client-delete', 'client-update']) --}}
                            <td class="text-center">
                                {{-- @can('client-update') --}}
                                <a href='#' wire:click.prevent="initData({{ $indicator->id }})"
                                    data-bs-toggle="modal" data-bs-target="#EditContratModal">
                        ...
                        <tr id="emptytr">
                            <td colspan="17">
                                <div class="text-center text-gray-800 mt-2">
                                    ...
                                    <h4 class="fs-5 fw-bold">{{ __('No Indicators') }} &#128540;</h4>
                                </div>
                            </td>
                        </tr>
                    @endforelse
