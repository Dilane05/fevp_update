<div wire:ignore.self class="modal side-layout-modal fade" id="ViewRecommendationModal" tabindex="-1" aria-labelledby="modal-form" style="display: none;" aria-hidden="true">
                ...
                <div class="p-3 p-lg-4">
                    <div class=" mt-md-0">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <h1 class="mb-0 h4">{{__('View Recommendation Details')}}</h1>
                                <p>{{__('view details of given Recommendation')}} &#128522;</p>
                            </div>
                            <div>
                                @if(!empty($complaint))
                                <a href="{{route('portal.recommendations.generate-pdf',['complaint_id' => $complaint->id])}}" class="btn btn-success mx-2" target="_blank">{{__('Generate PDF')}}</a>
                                @endif
                    ...
                    <div>
                        @if(!empty($complaint->recommendation))
                        <div class='row mt-2'>
                            <div class='col-8'>
                                <div class="accordion accordion-flush bg-white rounded-2 mb-3 card" id="accordionFlushExample">
                                    ...
                                    <div class="accordion-item">
                                        ...
                                        <div id="flush-collapseTwo" class="accordion-collapse collapse px-4 pb-3" aria-labelledby="flush-headingTwo" data-bs-parent="#accordionFlushExample">
                                ...
                                <div class='card '>
                                    <div class='card-body'>
                                        <div class='d-flex justify-content-between align-items-center'>
                                            <h5 class="fw-bold">{{__('Recommendations')}}</h5>
                                        </div>
                                        <hr>
                                        ...
                                        <div class="d-flex justify-content-between mb-2">
                                            <span>{{__('Start Date')}} {{$complaint->recommendation->start_date}}</span>
                                            <span>{{__('End Date')}} {{$complaint->recommendation->end_date}}</span>
                                        </div>
                                        <p class="fw-bolder">{{__('Recommendation Stages')}}</p>
                                        <hr>
                                        @foreach($complaint->recommendation->recommendation_types as $recommendation_type)

                                        <span class="fw-bold py-4">
                                            {{ucfirst($recommendation_type->recommendation_type_name)}} -
                                            {{__('From ')}} {{$recommendation_type->start_date->ISOFormat('LL')}} {{__(' to ')}}
                                            {{$recommendation_type->end_date->ISOFormat('LL')}}

                                        </span><br>

                                        <div class="py-3">
                                            @foreach($recommendation_type->recommendation_type_messages->take(5) as $recom_message)
                                            <div class='d-flex justify-content-between'>
                                                <span> <span class="fw-bold">{{__('Action : ')}}</span>{{$recom_message->message->name }}</span>
                                                <span> <span class="fw-bold"> {{$recom_message->delivery_time->format('H:i')}}</span></span>
                                        ...
                                        @endforeach
                                        @else
                                        <span>{{__('No recommendation')}}</span>
                                        @endif
