<div wire:ignore.self class="modal side-layout-modal fade" id="ViewClientReferralModal" tabindex="-1" aria-labelledby="modal-form" style="display: none;" aria-hidden="true">
                ...
                <div class="p-3 p-lg-4">
                    <div class=" mt-md-0">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <h1 class="mb-0 h4">{{__('View Referral Details')}}</h1>
                                <p>{{__('view referrals details of given Complaint')}} &#128522;</p>
                            </div>
                            <div>
                    ...
                    <div>
                        ...
                        <div class='row mt-2'>
                            <div class='col-md-8 col-sm-12 '>
                                ...
                                @if(!empty($referral->client_feedback) || !empty($referral->expert_feedback))
                                <div class='border px-4 pt-3 pb-2 rounded-2'>
                                    @if(!empty($referral->client_feedback))
                                    <div class='d-flex justify-content-between align-items-center'>
                                        <span class="fw-bolder ">{{__('Client Feedback')}}</span>
                                        <span>{{$referral->client_feedback_date}}</span>
                                    </div>
                                    <p>{!! $referral->client_feedback !!}</p>
                                    @if($referral->referral_results_file)
                                    <a href="{{asset('content/'.$referral->referral_results_file)}}" target="_blank" class="small mb-2 text-info">{{__('Download Result File')}}</a>
                                    @endif
                                    @endif
                                    @if(!empty($referral->expert_feedback))
                                    <hr class="my-1">
                                    <div class='d-flex justify-content-between align-items-center'>
                                        <span class="fw-bolder ">{{__('Expert Feedback')}}</span>
                                        <span>{{$referral->expert_feedback_date}}</span>
                                    </div>
                                    <p>{{ $referral->expert_feedback}}</p>
                                    @endif
                                </div>
                                @endif
                            </div>
                            <div class='col-md-4 col-sm-12'>
                                <div class='card mt-md-0 mt-sm-2 mt-xs-2 mb-2'>
                                    ...
                                    <div class='card-body '>
                                        <div class="">
                                            ...
                                            <a href="#" class="d-flex align-items-center">
                                                <div class="avatar avatar-md d-flex align-items-center justify-content-center fw-bold small fs-6 rounded bg-primary text-white  me-2"><span class="text-dark">{{$complaint->expert->initials}}</span></div>
                                                <div class="d-block"><span class="fw-bold smaller fs-6">{{ucwords($complaint->expert->name)}}</span>
